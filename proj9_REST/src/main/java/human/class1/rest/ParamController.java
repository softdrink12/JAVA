...
@Controller					// 컨트롤러 클래스에 붙여서 Bean으로 등록
						 // bean id를 지정하지 않으면
						 // calss 명의 앞글자가 소문자인 bean id가 자동 지정
						 // 그래서 지금은 "paramController"로 지정됨
//@Controller("bean id") // bean id를 지정하는 방법
@ResponseBody				// 이 class의 모든 메소드에 @ResponseBody를 붙여줌
// @RestController 			:@Controller + @ResponseBody 
@RequestMapping("/human/a")	// 주소 앞에 공통으로 적용
					// 이 주소로 Class까지 들어옴 (HandlerMapping)
					// 그리고 나머지 주소로 메소드 찾아감 (HandlerAdaptor)
public class ParamController {
	
	@Autowired		// 이 타입의 변수에 넣을 수 있는 동일한 클래스나 
					// 자동 형변환 되는 클래스를 찾아서 
					// 생성하고(IoC) 변수에 넣어준다(DI)
	// 만약 후보 클래스가 2개 이상 있다면
	// @Primary가 붙은 클래스가 온다 
	// @Primary가 없다면 @Qualifier("bean id")로 특정 bean을 지정할 수 있다
//	EmpSerive empService;
	...
	@ResponseBody	// json 또는 기본타입을 돌려줌, viewResolver를 거치지 않고 나감
	public EmpDTO parmTest(
			...
			// 필수일 때 null인 경우 400 Bad Request 발생
...
//			@RequestParam(value="pw", required=false) String pw
			...
			/*
			 * ex)
			 * Stirng ename = request.getParameter("ename")
			 * EmpDTo empDTo = new EMpDTO();
			 * empDTO.setEname(ename);
			 */
			
			@ModelAttribute("dto3") EmpDTO dto2,
			// 어차피 dto2를 model에 dto3라고 넣는 경우(model.addAttribute("dto3", dto2)을 한 번에 해줌
